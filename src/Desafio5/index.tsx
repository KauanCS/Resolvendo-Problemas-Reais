...
export function Desafio5() {
  const { list, addItem, parOuImpar, numero } = useDesafio5Controller();
  ...
  return (
    <>
      ...
      <ul>
        {list.map((item) => (
          <Item valor={item} key={item} parOuImpar={parOuImpar} />
        ))}
      </ul>
      <p>
...
/*
  Resposta: Utilização de hooks focados em performance, evitando 
  renderizações desnecessárias.
  
  Obs: foi necessário pequenas adaptações para passar no teste
*/
